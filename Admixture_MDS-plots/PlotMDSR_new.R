...
setwd("~/Work/plantago/Admixture_MDS-plots/")
...
colors = colorcode$V2
popn = colorcode$V1
countrycode = read.table("CountryCodes_MDS_plots.csv", header=T, sep=";", as.is=T)
colorcode$V3 = sapply(colorcode$V1, function(x) countrycode[which(countrycode$Site == x), 5] )
...
colorback = makeTransparent(colors, 50)
colorpop = tapply(colors, popn, function(x) { unique(x) } )
names(colorpop) = sapply(names(colorpop), function(x) countrycode[which(countrycode$Site == x), 5] )
...
text(mds[,1], mds[,2], label=colorcode$V3, cex=0.8, col=colorback, font=1)
